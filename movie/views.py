...
def movie_details(request, movie_id):
    try:
        ...
        shows = Show.objects.filter(movie=movie_id).order_by('theatre')
        ...
        if final_shows:
            show_by_theatre = []
            theatre = final_shows[0].theatre
            for i in range(0, len(final_shows)):
                if theatre != final_shows[i].theatre:
                    theatre = final_shows[i].theatre
                    show_list.append(show_by_theatre)
                    show_by_theatre = []
                show_by_theatre.append(final_shows[i])
